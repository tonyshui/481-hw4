...
module Inference = struct
  ...
  let update_boolvec_str (_s :string) size index bval : string =
    let (s :string) = if String.is_empty _s then String.make size '0' else _s in
    let s1 = Obj.magic s in 
    s1.[index] <- ((if bval then '1' else '0') : char) ;
    let s2 = Obj.magic s1 in 
    s2
